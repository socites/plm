function ItemProxy(item, id, session) {
    ...
    let getters = new ItemGetters(item, base);
    ...
    function onLoaded(data) {
        ...
        if (!base.loaded && typeof self.onLoaded === 'function') {

            let processed = self.onLoaded(data);
            processed = (typeof processed === 'boolean') ? processed : true;

            if (!processed) {
                console.warn('Item was not initialise', data);
                base.cache.clean();
                return;
            }
