function ItemProxy(item, id, session) {
    ...
    let getters = new ItemGetters(item, base, maps);
    ...
    function onFetched() {
        if (typeof self.onFetched === 'function') {
            self.onFetched();
        }
    }
    ...
    function load(params) {
        ...
        let promise = base.load(params, session);
        promise.then(onFetched);
        return promise;
    ...
    this.initialise = function (specs) {
        ...
        if (typeof specs !== 'object' || !(specs.fields instanceof Array) || !(specs.maps instanceof Array)) {
            console.error('Invalid parameter', specs);
            throw new Error('Invalid parameter');
        }

        base.data.define(specs.fields);
        maps.define(specs.maps);
