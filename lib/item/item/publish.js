function ItemPublish(base, registry) {
    ...
    let publishing;
    Object.defineProperty(base, 'publishing', {'get': () => !!publishing});

    let published;
    Object.defineProperty(base, 'published', {'get': () => !!published});

    let promise;

    base.publish = function (session) {

        if (promise) {
            throw new Error('Item is already being published');
        }
