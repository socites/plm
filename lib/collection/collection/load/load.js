function CollectionLoad(base) {
    ...
    base.load = function (params, limit, session, batcher) {
        ...
        let counters = (params.counters) ? params.counters : [];
        counters = (counters instanceof Array) ? counters : [counters];

        // Load counters.
        counters.map(function (counter) {

            if (!base.counters.has(counter)) {
                console.warn(`Counter "${counter}" does not exist`);
                return;
            }

            base.counters[counter].load(batcher);
        ...
        if (loaded && !specs.update) {
            ...
            if (typeof specs.fetch === 'undefined' || specs.fetch) {
                ...
                base.fetch(limit, session, specs.update, batcher).then(function () {
